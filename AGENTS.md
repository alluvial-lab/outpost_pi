...
This fork runs end-to-end on the dev VM (`codebox` = `192.168.50.110`). The
...
- **tailscale** — docker container `tailscale` (tailscale/tailscale,
  ...
  nodekey). Node `codebox` = `100.106.7.70`; advertises subnet route
  `192.168.50.0/24` (approved in the admin console) so the relay's LAN URL
  (`http://192.168.50.110:3300`) works both direct on the home LAN and
