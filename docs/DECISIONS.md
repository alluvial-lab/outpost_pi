...
| Decision | Current truth |
|---|---|
| **One active owner-channel generation per Owner key** | Secure channel material is keyed by the Owner public key. Attaching or re-pairing the same Owner key replaces the prior channel and peer record; a same-key Owner does not retain simultaneous independently keyed device channels. |
| **Session identity — restored canonical `session_id`** | The wire carries a canonical `session_id` on every session-scoped push message (`agent_chunk`, `agent_done`, `tool_request`, `tool_result`, `session_history`, etc.). The pi-extension stamps it (resolved by `RemoteSessionIssuer`, opaque to the relay); the app's `session_gate.dart` rejects missing/foreign session IDs (`missing_session_id`/`session_mismatch`/`active_session_unknown`) before any state mutation. This restores the designed-then-dropped invariant: the absorbed project's protocol spec *designed* `session_id` on every push message, it was dropped during MVP scoping ("1 pairing = 1 session"), and that produced the cross-session contamination class (a foreign `session_history` overwriting the active session's message box). `epic-bold-canonical-session` restored it: canonical, required, fail-closed, opaque to the relay. |
| **Reachability is one contract, not five** | The same backoff `[1, 2, 5, 10, 30]` and ping cadences are projected from a shared `Reachability` contract (`protocol/schema/reachability.json`) into the extension, app, and relay. `epic-bold-reachability-contract` lifted the app's `ConnectionStatus` sealed class and adopted it across all three via per-stack value/state-machine projections. |

## Wire protocol

| Decision | Current truth |
|---|---|
| **Generated protocol — one canonical schema** | The wire is defined once in a canonical JSON Schema (`protocol/schema/`) and projected into TS (`pi-extension/src/protocol/generated/protocol.generated.ts`: unions, validators, `decodeClient`/`decodeServer`, type-set registries), Dart (`app/lib/protocol/generated/protocol.g.dart`: sealed classes + `fromJson`), and Rust (`relay/src/protocol/generated/`: serde structs for outer envelope, cross-PC frames, room metadata). `types.ts`/`codec.ts` now dispatch over the generated helpers; the former `SERVER_TYPES` drift class is retired. Relay control/presence/rooms DTOs are generated in `app/lib/protocol/generated/relay_frames.g.dart`; `control_frames.dart` adapts those DTOs into app-domain models and is not a wire-shape island. The codegen is the cross-language contract test; any genuinely hand-maintained wire island must document its durable reason. |
...
| **4 MiB default message limit** | Relay rejects base64-decoded `ct` payloads above the limit. Configurable via `RELAY_MAX_CT_MIB` env (read once at startup in `relay/src/protocol/outer.rs`); defaults to 4 MiB. |
