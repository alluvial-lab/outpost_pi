...
**Break-it proof:** temporarily forcing the platform IME visibility query to return false made `focused real keyboard is never dismissed by the watchdog` fail with one forbidden `TextInput.hide`. Restoring the guard made the same test pass. No scratch mutation remains.
