...
**Notes**: Fast-lane story review. Implementation commit `69e6fa3` inspected; changed files match the scoped shell/category/component extraction. Category metadata now lives in `settings_category.dart`, the shell/nav derive from the registry, shared settings chrome is public within the settings feature, selected-category fallback and remote gating remain in `settings_page.dart`, and no `WorkspaceDocument` / `WorkspaceLayoutStore` / `PaneNode` imports were introduced under `cockpit/lib/app/settings/`. Verification attempted: `flutter analyze && flutter test` from `cockpit/` failed before analysis because `/opt/flutter/bin/cache` is read-only (`engine.stamp.tmp` / `engine.realm`). A direct `dart analyze` attempt was also not authoritative because `.dart_tool`/Flutter package resolution is unavailable in this checkout. No code-level blocker found in the reviewed diff.
