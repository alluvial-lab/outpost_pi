...
- `git show --stat 95725bc` — only owned files: `relay/src/handlers/pi_forward.rs` (comments), `relay/src/peers/registry.rs` (comments), `relay/tests/pi_forward_test.rs` (tests) + this story. No behavior change to forward path; no collision with other relay agents.
