...
class _ControllableChannel
    implements IChannel, IControlLink, IActiveRoomTarget {
  ...
  group('ConnectionManager — rooms (plan 17)', () {
    ...
    test(
      ...
      () async {
        ...
        final adoptEmit = cm.roomsStream.first.timeout(
          const Duration(seconds: 1),
        );
        ch.pushControl(
          const RoomsSnapshot(
            peer: 'epk_legacy_snapshot',
            rooms: [
              RoomInfo(
                roomId: 'main',
                name: 'main',
        ...
        final nextEmit = cm.roomsStream.first.timeout(
          const Duration(seconds: 1),
        );
        ch.pushControl(
          const RoomsSnapshot(
